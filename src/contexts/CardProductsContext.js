...
function CardProductsContextProvider({ children }) {
    ...
    const addInCard = useCallback((id) => {
        if (!user) {
            navigate('/login');
        } else {
            const product = (products.filter((cardProduct) => cardProduct.id === id))[0];
            product.isInCard = true;
            const CardProductsUpdated = [...cardProducts];
            CardProductsUpdated.push(product);
            setCardProducts(CardProductsUpdated);
        }
    }, [cardProducts, setCardProducts, user, navigate]);

    const removeInCard = useCallback((id) => {
        const product = (products.filter((cardProduct) => cardProduct.id === id))[0];
        product.isInCard = false;
        const ProductsCopy = [...cardProducts];
        const CardProductsUpdated = ProductsCopy.filter((cardProduct) => cardProduct.id !== id);
        setCardProducts(CardProductsUpdated);
    }, [cardProducts, setCardProducts]);

    const value = useMemo(function () {
        return { cardProducts, addInCard, removeInCard };
    }, [cardProducts, addInCard, removeInCard]);


    return (
        <CardProductsContext.Provider value={value}>
            {children}
        </CardProductsContext.Provider>
    );
}

export default CardProductsContextProvider;